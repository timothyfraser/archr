...
0. SETUP ################################ Load 00_entropy_utilities.R for h(), j(), i(), and ig().
...
Compute the entropy of a random variable that takes values 1,2, or 3 with equal probability.
...
This means that you can just multiply together the probabilities as your input.
...
Mutual Information. Execute the block and pay attention to the output it produces.
...
Compare decisions! Run these scripts getting a data.frame of architectures `archs` with `cost`, `benefit`, and `risk` metrics.
...
Check information gain from lots of features in this case we'll count each decision as a feature, as an example.
...
It highlights how entropy and tradespace insights supports the overall systems architecting process in this course.
