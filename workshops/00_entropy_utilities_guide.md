This tutorial complements `00_entropy_utilities.R` and unpacks the
workshop on information theory helpers. You will see how it advances the
...
- Navigate the script `00_entropy_utilities.R` within the Evaluation
  module.
- Connect the topic “Information theory helpers” to systems architecting
  decisions.
...
Create the helper function `h()` so you can reuse it throughout the
workshop.
...
Create the helper function `j()` so you can reuse it throughout the
workshop.
...
Create the helper function `i()` so you can reuse it throughout the
workshop.
...
Create the helper function `ig()` so you can reuse it throughout the
workshop.
...
Calculate entropy. Use the `%>%` operator to pass each result to the
next tidyverse verb.
...
**Learning Check 1.** What role does the helper `h()` defined in Step 1
...
It computes the Shannon entropy of a probability vector so you can
quantify baseline uncertainty.

</details>

**Learning Check 2.** What role does the helper `j()` defined in Step 2
play in this workflow?
...
It calculates the joint entropy across the `xy` distribution to feed
later comparisons.

</details>

**Learning Check 3.** What role does the helper `i()` defined in Step 3
play in this workflow?
...
It evaluates mutual information by contrasting the joint distribution
with the product of marginals.

</details>

**Learning Check 4.** What role does the helper `ig()` defined in Step 4
play in this workflow?
...
It orchestrates the information-gain workflow, collecting probabilities,
entropy splits, and a tidy summary.
