...
- Explore optimization routines configured with the `GA` package.

## Application

1. Open `workshops/46_optimization_time_series_case.R` in the Source pane and skim the header comments for context.
...
7. Review `apply` and `map` examples and try swapping in a different input vector to test yourself.
8. Inspect the optimization configuration (population size, mutation rate) to see how they influence GA output.
9. When you are comfortable, run `source(file.path("workshops", "46_optimization_time_series_case.R"))` or click Source to execute the full workshop.
10. Record key takeaways and questions in the Learning Checks section below.
