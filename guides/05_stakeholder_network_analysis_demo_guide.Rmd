...
It attaches dplyr, readr, ggplot2 and archr, ensuring their functions are available before you execute the downstream code.
...
It creates `link` that selects a focused set of columns, and threads the result through a dplyr pipeline. Create the object `link` so you can reuse it in later steps.
...
It creates `m`. Create the object `m` so you can reuse it in later steps.
...
It creates `g`. Execute the block and pay attention to the output it produces.
