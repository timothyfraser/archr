...
It creates `features` that builds a tibble of scenario data. Create the object `features` so you can reuse it in later steps.
...
It creates `a` that builds a tibble of scenario data. Suppose we have a sample of architectures.
...
It creates `archs` that adds derived columns, and threads the result through a dplyr pipeline. Is that architecture good?

</details>

**Learning Check 4.** What should you verify after chaining the pipeline in Step 15?
