...
It creates `grid` that adds derived columns, and selects a focused set of columns. Get full factorial grid of combinations.
...
It creates `data` that adds derived columns, and threads the result through a dplyr pipeline. We run the experiment once and get these results.
...
It creates `dbar_c` that computes summary statistics, and threads the result through a dplyr pipeline. Calculate the direct (one-way) treatment effects.
