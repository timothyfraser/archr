...
**Learning Check 1.** What role does the helper `get_cost()` defined in Step 3 play in this workflow?
...
It attaches dplyr, readr and archr, ensuring their functions are available before you execute the downstream code.
...
It creates `a` that builds a tibble of scenario data. Create the object `a` so you can reuse it in later steps.
...
It creates `m2_hours` that encodes conditional logic with `case_when()`. Create the object `m2_hours` so you can reuse it in later steps.
