...
It creates `decision1` that adds derived columns, and selects a focused set of columns. D1: Get a donut (Binary: 0, 1)?
...
It creates `part1` that filters rows to the cases of interest, and threads the result through a dplyr pipeline. Create the object `part1` so you can reuse it in later steps.
...
It creates `part2` that filters rows to the cases of interest, and threads the result through a dplyr pipeline. Create the object `part2` so you can reuse it in later steps.
