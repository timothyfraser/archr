...
It creates `a` that enumerates architecture combinations with `archr` helpers. Make architectures to work with.
...
It creates `m1` that encodes conditional logic with `case_when()`. When d1 is 1, make m1 3; When d1 is 0, make m1 0 and repeat for other problem Or, try using case when.
